...
class panel:
    def __init__(self, nname, nx, ny, nwidth, nheight, nmanager):
        self.name = nname
        # Store original unscaled coordinates
        self.original_x = nx
        self.original_y = ny
        self.original_width = nwidth
        self.original_height = nheight
        # Apply scaling for actual positioning
        self.x = scale_coord(nx)
        self.y = scale_coord(ny)
        self.width = scale_coord(nwidth)
        self.height = scale_coord(nheight)
...
class card_button(button):
    ...
    def __init__(self, nname, nx, ny, nwidth, nheight, nmanager, ncard): 
        self.card = ncard
        # Store original unscaled coordinates for repositioning
        self.original_x = nx
        self.original_y = ny
    ...
    def move_to(self, new_x, new_y):
        # Store the original unscaled coordinates
        self.original_x = new_x
        self.original_y = new_y
        
        # Scale for actual positioning
        self.x = scale_coord(new_x)
        self.y = scale_coord(new_y)

        centered_pos = (self.x - (self.width // 2), self.y - (self.height // 2))
...
def draw_startup():
    ...
    scaled_surface = pygame.transform.scale(image_surface, scale_size(1000, 250))

    title_text = pygame_gui.elements.UIImage(
        relative_rect=scaled_surface.get_rect(center=scale_pos(width//2, (height//2) - 100)),
...
def declare_images():
    ...
    startup_background_image = pygame.transform.scale(startup_background_image, (actual_width, actual_height))

    # draw startup background
    player_1_background_image = pygame.image.load("ASSETS/BACKGROUNDS/player_1_background.png").convert()
    # scale to window size
    player_1_background_image = pygame.transform.scale(player_1_background_image, (actual_width, actual_height))

    # draw startup background
    player_2_background_image = pygame.image.load("ASSETS/BACKGROUNDS/player_2_background.png").convert()
    # scale to window size
    player_2_background_image = pygame.transform.scale(player_2_background_image, (actual_width, actual_height))
...
# Reference resolution (1920x1080) - the resolution the game was designed for
...
manager = pygame_gui.UIManager((actual_width, actual_height), 'theme.json')
